...
import type { ProxyStatus, InterceptScript } from '@shared/types'
...
function findMitmdump(): string {
  ...
  if (configured && existsSync(configured as string)) return configured as string
...
type AddonLine =
  ...
  | { type: 'connection_error'; host: string; reason: string; error_type: string }
...
export class ProxyManager {
  ...
  getActivePort(): number | null { return this.activePort }
  ...
  private async killMitmproxyOnPort(port: number): Promise<{ 
    ...
    try {
      ...
      const lsofResult = execSync(`lsof -ti tcp:${port}`, { encoding: 'utf8' }).trim()
      ...
      for (const pidStr of pids) {
        ...
        try {
          ...
          const fullCommand = execSync(`ps -p ${pid} -o args=`, { encoding: 'utf8' }).trim()
          
          // Check if it's mitm-related (by name or full command)
          if (processName.includes('mitm') || processName.includes('mitmdump') ||
              fullCommand.includes('mitm') || fullCommand.includes('mitmdump')) {
            console.log(`[ProxyManager] Killing ${processName} (PID: ${pid}) on port ${port}`)
            execSync(`kill -9 ${pid}`)
            return { killed: true }
          }
  ...
  getScriptLogs(opts: { script?: string; level?: string; limit?: number } = {}): typeof this._scriptLogs {
  ...
  async start(sessionId: string, isRetry = false): Promise<{ port: number }> {
    if (this.status === 'running' && !isRetry) throw new Error('Proxy is already running')
    ...
    if (!isRetry) {
      ...
      if (cleanup.killed) {
        console.log(`[ProxyManager] Cleaned up existing mitmproxy on port ${port}`)
        // Wait for port to be released
        await new Promise(resolve => setTimeout(resolve, 500))
      } else if (cleanup.processName) {
        throw new Error(`Port ${port} is already in use by ${cleanup.processName} (PID: ${cleanup.pid}). Please stop that process or choose a different port.`)
      }
    }
    ...
    this.proc.on('exit', (code, signal) => {
      if (this.status !== 'stopped') {
        console.warn(`[ProxyManager] mitmdump exited unexpectedly: code=${code} signal=${signal}`)
        
        // If port conflict was detected, provide helpful error message
        if (this._portConflictDetected && this.startReject) {
          const error = new Error(`Failed to start proxy: Port ${port} is already in use. Please check if another proxy is running.`)
          this.cleanup()
          this.setStatus('error')
  ...
  sendScripts(scripts?: InterceptScript[]): void {
    ...
    const payload = JSON.stringify({ cmd: 'set_scripts', scripts: toSend }) + '\n'
    try {
  ...
  private handleAddonMessage(msg: AddonLine): void {
    switch (msg.type) {
      case 'ready': {
        ...
        this._portConflictDetected = false
        if (this.startResolve) {
          this.activePort = this.activePort ?? SettingsStore.getInstance().getProxySettings().port
          this.setStatus('running')
      ...
      case 'error': {
        console.error('[mitmdump addon error]', (msg as { type: string; message: string }).message)
      ...
      case 'connection_error': {
        const ce = msg as { type: string; host: string; reason: string; error_type: string }
      ...
      case 'script_error': {
        ...
        if (this._scriptLogs.length > ProxyManager.SCRIPT_LOG_MAX) {
          this._scriptLogs = this._scriptLogs.slice(-ProxyManager.SCRIPT_LOG_MAX)
        }
